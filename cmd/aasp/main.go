...
func (s *server) UnWrapKey(c context.Context, grpcInput *keyprovider.KeyProviderKeyWrapProtocolInput) (*keyprovider.KeyProviderKeyWrapProtocolOutput, error) {
	...
	retrieveVCertChain(azure_info.CertCache, &EncodedUvmInformation)
	...
	privkey, err := getRSAPrivateKey(&keyBytes)
	var plaintext []byte
	fmt.Println("wrapped data \n", string(annotation.WrappedData))
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Released key is not a RSA private key: %v", err)
	} else {
...
func getRSAPrivateKey(jwKey *jwk.Key) (*rsa.PrivateKey, error) {
	jwkJSONBytes, err := json.Marshal(jwKey)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "SKR marshalling failed: %v", err)
	}
	fmt.Println("priting out jwkJsonBytes\n", string(jwkJSONBytes))
	...
	if err := json.Unmarshal(jwkJSONBytes, &jwkData); err != nil {
		log.Fatal(err)
		return nil, status.Errorf(codes.Internal, "SKR unmarshalling failed: %v", err)
	}
...
func retrieveVCertChain(certCache attest.CertCache, encodedUvmInformation *common.UvmInformation) {
	RSASize := 2048
	...
	if err != nil {
		logrus.Debugf("decoding policy from Base64 format failed: %s", err)
	}
	...
	if err := SNPReport.DeserializeReport(SNPReportBytes); err != nil {
		logrus.Debugf("failed to deserialize attestation report: %s", err)
	}

	vcekCertChain, err := certCache.GetCertChain(SNPReport.ChipID, SNPReport.ReportedTCB)
	if err != nil {
		logrus.Debugf("retrieving cert chain from CertCache endpoint failed: %s", err)
	}
